...
项目已完成 Phase 0 / Phase 1 / Phase 2 / Phase 3 / Phase 4 / Phase 5 / Phase 6 的基础工程、真实文件验证、DWG 到 DXF 的本地转换能力、房间文字识别和 label 聚类、房间边界识别、初始房间 JSON 生成、PDF 矢量文字机器校核，以及 PDF 局部截图生成。
...
1. 进入下一阶段：OCR / 本地 AI 辅助校验。
2. 按 `LOCAL_AI_RUNTIME_SETUP.md` 读取本地 AI 运行配置。
3. 使用 `rooms_with_review_images.json` 中的截图，对 CAD/PDF 字段做局部视觉校验。
4. 将本地 AI 结果写入 evidence 和 issue，为后续置信度评分、review task 生成和正式人工校核做准备。
