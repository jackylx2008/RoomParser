...
项目已完成 Phase 0 / Phase 1 / Phase 2 / Phase 3 / Phase 4 的基础工程、真实文件验证、DWG 到 DXF 的本地转换能力、房间文字识别和 label 聚类、房间边界识别，以及初始房间 JSON 生成。
...
- 当前 `rooms_auto.json` 仍是 CAD 自动草稿，不是最终成果。
...
1. 进入 Phase 5：PDF 矢量文字校核。
2. 提取 PDF 文字和 bbox。
3. 建立 CAD 到 PDF 的坐标映射。
4. 将 `rooms_auto.json` 与 PDF 局部文字进行一致性比对。
5. 输出 `rooms_pdf_checked.json`，为后续局部截图、OCR / 本地 AI 校验和正式人工校核任务池做准备。
