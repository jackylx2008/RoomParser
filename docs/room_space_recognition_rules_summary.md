...
- 实验脚本：`experiments/room_recognition_experiment.py`
...
这些规则已合入正式 `build-room-candidates` 默认配置；日常真实房间识别不再需要通过 `experiments/room_recognition_experiment.py` 或手工重复传入同一组边界参数。
...
```powershell
python -m pytest -q
python experiments/room_recognition_experiment.py
```
...
- 若换用新 DXF，应优先复用 `experiments/room_recognition_experiment.py` 生成 auditable step 输出，再用 HTML 和本地 AI 共同核对。
