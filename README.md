...
- `room-extractor self-clean-dxf --source <file.dxf> --reference <ref.dxf> --out-dir <dir>` 运行正式 DXF 自驱动清理流程，默认保留 16 个小步阶段、rejected 阶段、HTML/PNG/删除记录和 rollback point。
...
- `room-extractor build-room-candidates --cad <cad_raw.json> --labels <room_label_candidates.json> --out <file>` 输出 `room_candidates.json`，将房间 label 中心点匹配到闭合 polygon；默认采用已验证的真实房间识别配置（`WALL`、`0-面积线`、`Defpoints` 等边界优先级，门洞补边，小墙缝拼接，结构柱边线参与），也支持 `--boundary-layer` 显式覆盖房间边界图层优先级；支持 `--axes` / `--columns` 注入轴线和柱子 JSON 上下文。
...
- `python dxf_preparation.py ...`：只暴露 Workflow A 命令，例如 `convert-dwg`、`explode-dxf`、`dedupe-dxf-lines`、`self-clean-dxf`。
...
针对炸块后 DXF 中大量非几何数据导致文件膨胀、计算量增加和房间识别效果下降的问题，正式预处理 workflow 已合入自驱动清理命令：

```powershell
room-extractor self-clean-dxf --source data/input/dxf_exploded/test.dxf --reference data/input/dxf_exploded/test1.dxf --out-dir log/dxf_cleaning_experiment --dry-run-ai
```

也可以从项目根目录使用 workflow 专用入口：

```powershell
python dxf_preparation.py self-clean-dxf --resume log/dxf_cleaning_experiment --rollback-to 15
```

该命令以一个臃肿 DXF 和一个视觉等价的小 DXF 为参考，按小步迭代清理：
...
- 默认最多执行全部 16 个已验证阶段；第 004 步 `rebuild_visible_modelspace` 作为历史 rejected 阶段保留，不会更新 `current.dxf`。
...
默认配置已经合入房间识别实验通过的墙体边界、门洞补边、小墙缝拼接和结构柱上下文规则。仍可显式指定房间边界图层和优先级，并注入轴线 / 柱子 JSON 上下文：
