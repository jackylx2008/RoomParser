...
阶段开发验收用于判断当前 Phase 的中间产物是否具备继续开发下一 Phase 的条件。例如 Phase 3 完成后，可以输出 CAD 底图 + 房间 polygon + label 状态叠加的可视化检查图，用于检查房间边界识别规则是否明显偏离图纸。阶段开发验收不是最终业务人工校核，不应直接产出最终人工确认结果。

阶段开发验收不能只依赖人工阅读 JSON。涉及几何、边界、坐标、图纸位置的阶段，必须提供可视化产物，例如 HTML/SVG 叠图、PDF 截图叠图或 CAD 导出标注图。JSON 只作为证据链和机器可读中间结果。

第二类是正式人工校核。

正式人工校核应放在机器校验之后，即 CAD 自动提取、PDF 矢量文字校核、局部截图生成、OCR / 本地 AI 辅助识别、置信度评分和 issue 分类完成后，再由人工处理低置信度或冲突数据。正式人工校核结果应写入 review record，并按“人工校核结果 > PDF 矢量校核结果 > CAD 自动提取结果 > OCR / VLM 辅助结果”的优先级回写最终 JSON。

阶段开发验收发现的问题，应按性质进入不同处理路径：

1. 数据修正
   - 仅影响单个房间或单个图纸结果。
   - 不在早期 Phase 直接改最终结果，应在后续正式人工校核或回写阶段处理。
2. 规则缺陷
   - 说明当前正则、图层规则、聚类规则、边界匹配规则或置信度规则存在可复用问题。
   - 必须调整代码或配置，并重新生成该阶段中间 JSON。
3. 样本沉淀
   - 将人工发现的典型问题抽象为脱敏测试样例、规则样本或训练样本。
   - 后续用于防止回归和优化规则。
...
1. 阶段开发验收只确认当前阶段规则和中间产物是否可继续，不替代正式人工校核。
2. 正式人工校核必须排在机器校验、置信度评分和 review task 生成之后。
3. 人工确认结果的优先级高于自动规则。
4. 自动规则不得覆盖人工确认结果。
5. 被人工确认的问题如果具有复用价值，应沉淀为测试、配置或规则文档。
6. 每个阶段的中间 JSON 必须保留足够 evidence、issue 和 summary，便于阶段开发验收和规则回归。

阶段开发验收与规则回归流程图：

```mermaid
flowchart TD
    A[开始当前 Phase 开发] --> B[实现代码与配置]
    B --> C[运行单元测试]
    C --> D{测试是否通过}
    D -- 否 --> B
    D -- 是 --> E[用真实图纸生成阶段中间 JSON]
    E --> F[生成可视化检查图和 summary]
    F --> G[阶段开发验收]
    G --> H{阶段验收是否通过}
    H -- 是 --> I[冻结当前 Phase 输出口径]
    I --> J[进入下一 Phase]
    H -- 否 --> K[归类阶段反馈]
    K --> L{反馈类型}
    L -- 数据修正 --> M[记录为后续正式人工校核输入]
    L -- 规则缺陷 --> N[调整代码或配置规则]
    L -- 样本沉淀 --> O[补充脱敏测试样例或规则样本]
    M --> P[重新生成中间 JSON]
    N --> C
    O --> C
    P --> F
...
flowchart TD
    ...
    B --> C[CAD/PDF 坐标映射与局部文字比对]
    C --> D[生成 PDF 局部截图]
    D --> E[OCR / 本地 AI 辅助校验]
    E --> F[置信度评分与 issue 分类]
    F --> G{是否高置信度且无严重 issue}
    G -- 是 --> H[auto_passed]
    G -- 否 --> I[生成 review_tasks.json]
    I --> J[正式人工校核界面]
    J --> K[人工修正文字或绘制 polygon]
    K --> L[生成 review record 和 manual polygon]
    L --> M[人工结果回写 rooms_final.json]
```
