...
### 6. 每个开发阶段必须经过人工验收与规则回归调整

本项目按 Phase 递进开发，但每个 Phase 不是一次性完成后直接冻结。

每个 Phase 完成后，必须先输出该阶段的中间 JSON，并使用真实图纸进行人工验收。人工验收发现的问题，应按性质进入不同处理路径：

1. 数据修正
   - 仅影响单个房间或单个图纸结果。
   - 后续通过人工校核记录、manual polygon 或最终 JSON 回写处理。
2. 规则缺陷
...
1. 人工确认结果的优先级高于自动规则。
2. 自动规则不得覆盖人工确认结果。
3. 被人工确认的问题如果具有复用价值，应沉淀为测试、配置或规则文档。
4. 每个阶段的中间 JSON 必须保留足够 evidence、issue 和 summary，便于人工判断和规则回归。
...
flowchart TD
    ...
    E --> F[人工验收中间 JSON]
    F --> G{人工是否确认通过}
    G -- 是 --> H[冻结当前 Phase 输出口径]
    H --> I[进入下一 Phase]
    G -- 否 --> J[归类人工反馈]
    J --> K{反馈类型}
    K -- 数据修正 --> L[记录人工修正或待校核任务]
    K -- 规则缺陷 --> M[调整代码或配置规则]
    K -- 样本沉淀 --> N[补充脱敏测试样例或规则样本]
    L --> O[重新生成中间 JSON]
    M --> C
    N --> C
    O --> F
